...
def get_device(db: Session = Depends(get_postg_db)):
...
def notify_users(modification: Modification, db: Session = Depends(get_postg_db)):
    modification_encoded = jsonable_encoder(modification)
    notified_users = notification_manager.notify_modifications(modification_encoded, db)
